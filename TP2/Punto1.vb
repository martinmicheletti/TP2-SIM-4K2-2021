...
Public Class Punto1
    ...
    Dim sum As Integer = 0 ''frecuencia observada por intervalo
    Dim media_intervalo As Double
    Dim fileReader
    Dim lista_pesos As New List(Of Double)
    Dim k As Integer
    ...
    Private Sub graficarBTN_Click(sender As Object, e As EventArgs) Handles graficarBTN.Click
        ...
        Else
            ...
            fileReader = File.OpenText("C:\Users\Martin\Downloads\TP2_Constantini, Loza, Micheletti, Soaje (1)\TP2_Constantini, Loza, Micheletti, Soaje\Pesos_Sudeste_Asia.txt")

            While fileReader.Peek <> -1
                Dim peso As Double
                peso = fileReader.ReadLine()

                lista_pesos.Add(peso)

            End While
            Dim listaValores(k - 1) As Double
            intervalos = (DeterminarMayor(lista_pesos) - DeterminarMenor(lista_pesos)) / k

            intervalos = Math.Round(intervalos, 4) + 0.001

            For i As Double = DeterminarMenor(lista_pesos) To DeterminarMayor(lista_pesos) Step intervalos
                sum = 0
                acum_dif_media = 0
                acum = 0
                For count = 0 To (lista_pesos.Count - 1) ''iteramos desde 0 hasta la cantidad de muestras solicitadas
                    If i <= lista_pesos(count) Then ''si el numero pseudo-aleaotrio es mayor que el limite inferior del intervalo 
                        If lista_pesos(count) < (i + intervalos) Then ''y menor que el limite superior del intervalo
                            sum = sum + 1 ''incremento de la frecuencia de aparicion en dicho intervalo
                        End If
                    End If
                    acum = acum + lista_pesos(count)
                    sum = Math.Round(sum, 4)
                    i = Math.Round(i, 4)
                Next count

                frecuenciaObsAcumulada = Math.Round(frecuenciaObsAcumulada + sum, 4)
                media = Math.Round(acum / lista_pesos.Count, 4)

                probObs = sum / lista_pesos.Count ''probabilidad observada de cada intevalo
                probObsAcum = Math.Round(probObsAcum + probObs, 4) ''probabilidad observada
                probEsperada = (lista_pesos.Count / k) / lista_pesos.Count
                probEspAcum = Math.Round((probEspAcum + probEsperada), 4)
                For count = 0 To (lista_pesos.Count - 1)
                    Dim diff As Double = 0

                    diff = Math.Pow((lista_pesos(count) - media), 2)
                ...
                var = acum_dif_media / lista_pesos.Count
                desv = Math.Sqrt(var)
                ...
                fe = (numerador / denominador) * intervalos * lista_pesos.Count
            ...
            lst_SerieDatos.DataSource = lista_pesos
            ...
            ''
            testKolmogorov = 1.36 / Math.Sqrt(lista_pesos.Count) '' 1/raizcuadrada(n) ---> nivel de significación de alfa 0.05
